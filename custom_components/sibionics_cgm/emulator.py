...
class CalibrationEngine:
    ...
    def _dispatch_hook(self, name: str, uc: Uc, pc: int, lr: int) -> None:
        ...
        if name == "pow":
            ...
            v = self._read_d(uc, UC_ARM64_REG_D0)
            try:
                self._write_d(uc, UC_ARM64_REG_D0, math.pow(2.0, v))
            except OverflowError:
                self._write_d(uc, UC_ARM64_REG_D0, float("inf"))
        elif name == "log":
            v = self._read_d(uc, UC_ARM64_REG_D0)
            ...
            v = self._read_d(uc, UC_ARM64_REG_D0)
            self._write_d(uc, UC_ARM64_REG_D0, math.log2(v) if v > 0 else float("-inf"))
        elif name == "log10":
            v = self._read_d(uc, UC_ARM64_REG_D0)
            self._write_d(uc, UC_ARM64_REG_D0, math.log10(v) if v > 0 else float("-inf"))
        elif name == "fabs":
            self._write_d(uc, UC_ARM64_REG_D0, math.fabs(self._read_d(uc, UC_ARM64_REG_D0)))
        elif name == "ceil":
            self._write_d(uc, UC_ARM64_REG_D0, math.ceil(self._read_d(uc, UC_ARM64_REG_D0)))
        elif name == "floor":
            self._write_d(uc, UC_ARM64_REG_D0, math.floor(self._read_d(uc, UC_ARM64_REG_D0)))
        elif name in ("round", "rint", "nearbyint"):
            self._write_d(uc, UC_ARM64_REG_D0, round(self._read_d(uc, UC_ARM64_REG_D0)))
        elif name == "sin":
            self._write_d(uc, UC_ARM64_REG_D0, math.sin(self._read_d(uc, UC_ARM64_REG_D0)))
        elif name == "cos":
            self._write_d(uc, UC_ARM64_REG_D0, math.cos(self._read_d(uc, UC_ARM64_REG_D0)))
        elif name == "tan":
            self._write_d(uc, UC_ARM64_REG_D0, math.tan(self._read_d(uc, UC_ARM64_REG_D0)))
        elif name == "asin":
            v = self._read_d(uc, UC_ARM64_REG_D0)
            self._write_d(uc, UC_ARM64_REG_D0, math.asin(max(-1.0, min(1.0, v))))
        elif name == "acos":
            v = self._read_d(uc, UC_ARM64_REG_D0)
            self._write_d(uc, UC_ARM64_REG_D0, math.acos(max(-1.0, min(1.0, v))))
        elif name == "atan":
            self._write_d(uc, UC_ARM64_REG_D0, math.atan(self._read_d(uc, UC_ARM64_REG_D0)))
        elif name == "atan2":
            y = self._read_d(uc, UC_ARM64_REG_D0)
            x = self._read_d(uc, UC_ARM64_REG_D1)
            self._write_d(uc, UC_ARM64_REG_D0, math.atan2(y, x))
        elif name == "fmod":
            x = self._read_d(uc, UC_ARM64_REG_D0)
            y = self._read_d(uc, UC_ARM64_REG_D1)
            self._write_d(uc, UC_ARM64_REG_D0, math.fmod(x, y) if y != 0 else float("nan"))
        elif name == "hypot":
            x = self._read_d(uc, UC_ARM64_REG_D0)
            y = self._read_d(uc, UC_ARM64_REG_D1)
            self._write_d(uc, UC_ARM64_REG_D0, math.hypot(x, y))
        elif name == "cbrt":
            v = self._read_d(uc, UC_ARM64_REG_D0)
            self._write_d(uc, UC_ARM64_REG_D0, math.copysign(abs(v) ** (1.0 / 3.0), v))
        elif name == "copysign":
            x = self._read_d(uc, UC_ARM64_REG_D0)
            y = self._read_d(uc, UC_ARM64_REG_D1)
            self._write_d(uc, UC_ARM64_REG_D0, math.copysign(x, y))
        elif name == "ldexp":
            x = self._read_d(uc, UC_ARM64_REG_D0)
